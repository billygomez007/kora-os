...
describe('StaffInvitationEmailService with Resend', () => {
  ...
  it('uses the preferred production web origin when KORA_WEB_URL is omitted', async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response('{}', { status: 200 }));
    vi.stubGlobal('fetch', fetchMock);
    const service = new StaffInvitationEmailService(
      new ConfigService({
    ...
    const request = fetchMock.mock.calls[0][1] as RequestInit;
    const body = JSON.parse(String(request.body));
    expect(body.text).toContain('https://www.koraafric.com/invite/invite-token-value');
    expect(body.html).toContain('https://www.koraafric.com/invite/invite-token-value');
  });
