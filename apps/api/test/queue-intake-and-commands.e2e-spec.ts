...
import {
  extendWithQueueRoles,
  type QueueFixtureExtras,
} from './support/queue-test-fixtures.js';
...
function sameDayNearFutureSlotStart(): Date {
  ...
  throw new Error(
    'No same-day near-future slot is available this close to Africa/Accra midnight — re-run shortly.',
  );
}
...
describe('Walk-in intake, appointment check-in, and queue commands (e2e)', () => {
  ...
  describe('walk-in intake', () => {
      ...
      const response = await authed(testApp, extras.receptionistAccessToken)
        ...
        .send(
          walkInPayload({
            ...
            customerRecordId: extras.customerRecordId,
          }),
        )
        .expect(201);
    ...
    it('rejects a cross-tenant customerRecordId', async () => {
      ...
      const response = await authed(testApp, extras.receptionistAccessToken)
        ...
        .send(
          walkInPayload({
            newCustomer: undefined,
            customerRecordId: otherCustomer.id,
          }),
        );
    ...
    it('rejects a branch belonging to another organization', async () => {
      ...
      const response = await authed(testApp, fixture.ownerAccessToken)
        .post(
          `/v1/organizations/${fixture.organizationId}/branches/${other.branchId}/queue/walk-ins`,
        )
        .send(walkInPayload());
    ...
    it('a repeated request with the same Idempotency-Key returns the original queue entry', async () => {
      ...
      const count = await testApp.prisma.queueEntry.count({
    ...
    it('several concurrent walk-ins receive unique, sequential ticket numbers', async () => {
      const attempts = await Promise.all(
        Array.from({ length: 6 }, (_, index) =>
          authed(testApp, extras.receptionistAccessToken)
            ...
            .send(
              walkInPayload({
                newCustomer: { name: `Concurrent Walk-in ${index}` },
              }),
            ),
        ),
      );
      ...
      const ticketNumbers = attempts
        .map((response) => response.body.data.ticketNumber)
        .sort((a, b) => a - b);
  ...
  describe('appointment check-in', () => {
    async function bookConfirmedAppointment(
      startAt: Date = sameDayNearFutureSlotStart(),
    ) {
      const response = await authed(testApp, fixture.ownerAccessToken)
        .post(
          `/v1/organizations/${fixture.organizationId}/branches/${fixture.branchId}/appointments`,
        )
        .send({
          serviceIds: [fixture.serviceId],
    ...
    it('checks a confirmed same-day appointment into the queue, carrying its provider and services', async () => {
      ...
      expect(response.body.data.status).toBe('WAITING');
      expect(response.body.data.assignedStaffProfileId).toBe(
        fixture.providerStaffProfileId,
      );
      expect(
        response.body.data.services.map(
          (s: { serviceId: string }) => s.serviceId,
        ),
      ).toEqual([fixture.serviceId]);

      const appointmentAfter =
        await testApp.prisma.appointment.findUniqueOrThrow({
    ...
    it('rejects checking in a cancelled appointment', async () => {
      ...
      await authed(testApp, fixture.ownerAccessToken)
        .post(
          `/v1/organizations/${fixture.organizationId}/branches/${fixture.branchId}/appointments/${appointment.id}/cancel`,
        )
        .send({})
        .expect(201);
      ...
      const response = await authed(
        testApp,
        extras.receptionistAccessToken,
      ).post(checkInUrl(appointment.id));
      expect(response.status).toBe(409);
    ...
    it('rejects checking in an appointment scheduled for a different branch-local date', async () => {
      ...
      const futureAppointment = await testApp.prisma.appointment.create({
        data: {
          ...
          startAt: tomorrow,
          endAt: new Date(
            tomorrow.getTime() + fixture.serviceDurationMinutes * 60_000,
          ),
          occupiedStartAt: tomorrow,
          occupiedEndAt: new Date(
            tomorrow.getTime() + fixture.serviceDurationMinutes * 60_000,
          ),
      ...
      const response = await authed(
        testApp,
        extras.receptionistAccessToken,
      ).post(checkInUrl(futureAppointment.id));
      expect(response.status).toBe(400);
    });

    it('rejects checking in an appointment from another organization', async () => {
      const other = await createBookableFixture(testApp);
      const otherAppointmentResponse = await authed(
        testApp,
        other.ownerAccessToken,
      )
        .post(
          `/v1/organizations/${other.organizationId}/branches/${other.branchId}/appointments`,
        )
        .send({
      ...
      const response = await authed(
        testApp,
        extras.receptionistAccessToken,
      ).post(checkInUrl(otherAppointmentResponse.body.data.id));
      expect(response.status).toBe(404);
    });
    ...
    it('several concurrent check-in requests for the same appointment create exactly one queue entry', async () => {
      ...
      const attempts = await Promise.all(
        Array.from({ length: 5 }, () =>
          authed(testApp, extras.receptionistAccessToken).post(
            checkInUrl(appointment.id),
          ),
        ),
      );
  ...
  describe('queue commands and state machine', () => {
    ...
    it('moves WAITING -> CALLED -> IN_SERVICE is not reachable manually, but CALLED -> WAITING is', async () => {
      ...
      const called = await authed(testApp, extras.receptionistAccessToken)
        .post(
          `/v1/organizations/${fixture.organizationId}/queue-entries/${entry.id}/call`,
        )
        .expect(201);
      expect(called.body.data.status).toBe('CALLED');
      expect(called.body.data.calledAt).not.toBeNull();

      const backToWaiting = await authed(
        testApp,
        extras.receptionistAccessToken,
      )
        .post(
          `/v1/organizations/${fixture.organizationId}/queue-entries/${entry.id}/return-to-waiting`,
        )
        .expect(201);
    ...
    it('rejects an invalid transition (cancelling an already-cancelled entry)', async () => {
      ...
      await authed(testApp, extras.receptionistAccessToken)
        .post(
          `/v1/organizations/${fixture.organizationId}/queue-entries/${entry.id}/cancel`,
        )
        .send({})
        .expect(201);
    ...
    it('marks a waiting entry no-show', async () => {
      ...
      const response = await authed(testApp, extras.receptionistAccessToken)
        .post(
          `/v1/organizations/${fixture.organizationId}/queue-entries/${entry.id}/no-show`,
        )
        .expect(201);
    ...
    it('assigns an eligible provider and rejects an ineligible one', async () => {
      ...
      const assigned = await authed(testApp, extras.receptionistAccessToken)
        .post(
          `/v1/organizations/${fixture.organizationId}/queue-entries/${entry.id}/assign`,
        )
        .send({ staffProfileId: fixture.providerStaffProfileId })
        .expect(201);
      expect(assigned.body.data.assignedStaffProfileId).toBe(
        fixture.providerStaffProfileId,
      );
    ...
    it('creates an audit event for a walk-in and for a call command', async () => {
      ...
      expect(events.map((e) => e.action)).toEqual(
        expect.arrayContaining(['queue.walk_in.created', 'queue.entry.called']),
      );
    });

    it('bumps the branch queue day revision on every mutation', async () => {
      const entryResponse = await authed(
        testApp,
        extras.receptionistAccessToken,
      )
        .post(walkInUrl())
        .send(walkInPayload())
        .expect(201);
      const listAfterCreate = await authed(
        testApp,
        extras.receptionistAccessToken,
      )
        .get(
          `/v1/organizations/${fixture.organizationId}/branches/${fixture.branchId}/queue`,
        )
        .expect(200);
      ...
      await authed(testApp, extras.receptionistAccessToken)
        .post(
          `/v1/organizations/${fixture.organizationId}/queue-entries/${entryResponse.body.data.id}/call`,
        )
        .expect(201);

      const listAfterCall = await authed(
        testApp,
        extras.receptionistAccessToken,
      )
        .get(
        ...
        .expect(200);
      expect(listAfterCall.body.data.revision).toBeGreaterThan(
        revisionAfterCreate,
      );
    });
  });
  ...
  describe('queue listing', () => {
    async function createAssignedEntry(name: string, staffProfileId: string) {
      const created = await authed(testApp, extras.receptionistAccessToken)
        .post(walkInUrl())
        .send(walkInPayload({ newCustomer: { name } }))
        .expect(201);
      await authed(testApp, extras.receptionistAccessToken)
        .post(
          `/v1/organizations/${fixture.organizationId}/queue-entries/${created.body.data.id}/assign`,
        )
        .send({ staffProfileId })
        .expect(201);
    ...
    it('orders by priority, then join time, then ticket number, and reports counts', async () => {
      ...
      const priority = await authed(testApp, extras.receptionistAccessToken)
        ...
        .send(
          walkInPayload({
            newCustomer: { name: 'Priority Customer' },
            priority: 'PRIORITY',
          }),
        )
        .expect(201);
      ...
      const orderedIds = list.body.data.entries.map(
        (e: { id: string }) => e.id,
      );
      expect(orderedIds).toEqual([
        priority.body.data.id,
        normal1.body.data.id,
        normal2.body.data.id,
      ]);
    ...
    it('filters by status', async () => {
      ...
      await authed(testApp, extras.receptionistAccessToken)
        .post(
          `/v1/organizations/${fixture.organizationId}/queue-entries/${entry.body.data.id}/cancel`,
        )
        .send({})
        .expect(201);
      ...
      const waitingList = await authed(testApp, extras.receptionistAccessToken)
        .get(
          `/v1/organizations/${fixture.organizationId}/branches/${fixture.branchId}/queue?status=WAITING`,
        )
        .expect(200);
    ...
    it('preserves the full queue for an owner', async () => {
      const own = await createAssignedEntry(
        'Owner sees provider one',
        fixture.providerStaffProfileId,
      );
      const other = await createAssignedEntry(
        'Owner sees provider two',
        extras.secondProviderStaffProfileId,
      );
      ...
      expect(
        list.body.data.entries.map((entry: { id: string }) => entry.id),
      ).toEqual(expect.arrayContaining([own.id, other.id]));
      expect(list.body.data.counts.waiting).toBe(2);
    });

    it('forces a provider list and counts to their own StaffProfile', async () => {
      const own = await createAssignedEntry(
        'Provider own customer',
        fixture.providerStaffProfileId,
      );
      const other = await createAssignedEntry(
        'Other provider customer',
        extras.secondProviderStaffProfileId,
      );
      await authed(testApp, extras.receptionistAccessToken)
        .post(
          `/v1/organizations/${fixture.organizationId}/queue-entries/${other.id}/call`,
        )
        .expect(201);

      const list = await authed(testApp, fixture.providerAccessToken)
        .get(
          `/v1/organizations/${fixture.organizationId}/branches/${fixture.branchId}/queue`,
        )
        .expect(200);
      ...
      expect(list.body.data.counts).toMatchObject({ waiting: 1, called: 0 });
    });

    it('ignores a restricted provider filter that names another provider', async () => {
      const own = await createAssignedEntry(
        'Provider cannot bypass scope',
        fixture.providerStaffProfileId,
      );
      await createAssignedEntry(
        'Still hidden',
        extras.secondProviderStaffProfileId,
      );
      ...
      const list = await authed(testApp, fixture.providerAccessToken)
        .get(
          `/v1/organizations/${fixture.organizationId}/branches/${fixture.branchId}/queue?assignedStaffProfileId=${extras.secondProviderStaffProfileId}`,
        )
        .expect(200);
      ...
      expect(list.body.data.counts.waiting).toBe(1);
    });
  });

  describe('queue entry reads', () => {
    async function createAssignedEntry(staffProfileId: string) {
      const created = await authed(testApp, extras.receptionistAccessToken)
        .post(walkInUrl())
        .send(walkInPayload())
        .expect(201);
    ...
    it('lets a provider read their own assigned entry', async () => {
      const entry = await createAssignedEntry(fixture.providerStaffProfileId);
      await authed(testApp, fixture.providerAccessToken)
        .get(
          `/v1/organizations/${fixture.organizationId}/queue-entries/${entry.id}`,
        )
        .expect(200);
    });

    it("hides another provider's entry from a provider", async () => {
      const entry = await createAssignedEntry(
        extras.secondProviderStaffProfileId,
      );
      await authed(testApp, fixture.providerAccessToken)
        .get(
    ...
    it('lets an owner read an accessible entry', async () => {
      const entry = await createAssignedEntry(
        extras.secondProviderStaffProfileId,
      );
      await authed(testApp, fixture.ownerAccessToken)
