...
- Android now calls the real API for two vertical slices — customer discovery/booking plus an initial business dashboard, and business owner onboarding/team management (see both Android-integration bullets above); it does not yet call the walk-in/queue, service-session, checkout, payment, transaction, refund/reversal, cash-session, or commission-management endpoints, nor branch-service price/duration overrides, staff-service assignment, schedule exceptions, booking policy, staff availability rules/exceptions, invitation resend/reissue, or custom-role management — only discovery, availability, appointments, favorites, workspaces, reports overview, organization onboarding, business profile, service catalogue (create/archive), weekly business hours, and staff invitations.
...
- Commission accrual, service receipts, owner/manager reports, branch cash-session reconciliation, and refund/reversal corrections are all implemented on the backend (see the two "financial-integrity"/"cash controls" bullets above) but not reachable from Android yet; payouts, commission "paid"/settlement status, payment-gateway integration, and external settlement matching do not exist anywhere yet.
...
14. Android API integration. **Started, out of sequence order, ahead of items 9–13** — see both Android-integration bullets under "Current baseline" above for exactly what is and is not connected. Remaining for this item: queue, service-session, checkout, payment, refund, cash-session, and commission operations on Android; branch-service overrides, staff-service assignment, schedule exceptions, booking policy, staff availability rules/exceptions; invitation resend/reissue; custom-role management; a verified HTTPS Android App Link for the invitation deep link; automated invitation-email delivery; push notifications; offline synchronization.
...
are done — see "Current baseline" above. The first production Android
integration is also done for the slice it covers: passwordless sign-in,
session restoration, secure workspace selection, customer discovery and
booking, appointment management, favorites, and an initial
permission-gated business dashboard (backed by two small additive
endpoints, `GET /v1/me/workspaces` and the `/v1/me/favorites` family —
docs/API_SPEC.md sections 31-32) — started ahead of items 9-13 in the
implementation sequence by explicit product direction, not because the
backend's own remaining boundary changed. The next backend step remains
item 9 of the implementation sequence: subscription billing-provider
integration. The next Android step is connecting the remaining
business-operation surface (queue, service sessions, checkout, payments,
refunds, cash sessions, commissions) once its turn in the sequence is
reached, plus push notifications and offline synchronization (items
10-13). No further feature module is implemented until each prior one's
