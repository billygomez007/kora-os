...
- Commission accrual, service receipts, and owner/manager reporting — the next stage after a `Transaction` posts. Versioned `CommissionRule`s (PERCENTAGE/FIXED/NONE, independently scoped by branch/staff/service, an eight-level precedence, superseded rather than edited in place, unique-per-scope enforced by a partial `NULLS NOT DISTINCT` database index) resolve to an immutable `CommissionAccrual` per line item — a zero-value `NO_POLICY` accrual when nothing matches, never a blocked posting. An immutable `Receipt` (`{branchCode}-{year}-{sequence}` numbering, an atomic per-branch/year counter) is issued in the same database transaction as the accruals and the `Transaction` itself — the full chain commits or rolls back together. Endpoints under `.../commission-rules{,/:id,/:id/supersede,/:id/deactivate}`, `.../commissions`, `.../me/earnings`, `.../receipts{,/:id}`, `/me/receipts{,/:id}`, and `.../reports/{overview,revenue,staff-performance,services,payment-methods,commissions}` (docs/API_SPEC.md sections 20-22), gated by the new `commissions.read_own`/`.read_all`/`.manage`, `receipts.read`, and `reports.read` permissions. Every report figure derives only from a POSTED `Transaction` and its snapshots — a RECORDED/DISPUTED payment claim never inflates revenue, and every monetary total stays strictly separated by currency (docs/SECURITY.md section 34, docs/ARCHITECTURE.md section 21). Cash-session reconciliation, refunds/reversals, payouts, and commission "paid" status remain unimplemented — deferred to a later phase.
...
- Commission accrual, service receipts, and owner/manager reports are implemented (see the "financial-integrity core" bullet's follow-on above); cash-session reconciliation, refunds, reversals, payouts, and any "paid" status for a commission do not exist yet, and a posted `Transaction` cannot currently be reversed or refunded once created.
- Android is the only implemented client, and does not yet call any of the service-catalogue, availability, appointment, queue, service-session, checkout, payment, transaction, commission, receipt, or report endpoints above.

### Implementation sequence for the remaining work

Kept intentionally concise — each item expands into its own phase below once it starts, and is not built ahead of that phase. Items 1–8 (authentication/sessions/staff invitations; organization-scoped RBAC, branch authorization, and subscription enforcement; services, staff availability, and customer appointment booking; walk-ins/live queue; service sessions; checkout and line items; payments, provider verification, and disputes; commissions, receipts, and reports) and public business discovery are done — see "Current baseline" above — so the active boundary starts at item 9:
...
8. ~~Commission accrual, service receipts, and owner/manager reports.~~ Done — see "Current baseline" above. Cash-session reconciliation remains a carry-over gap, alongside refunds/reversals deferred out of item 7.
...
**Status: commissions, receipts, and reports done; reconciliation and refunds/reversals still not implemented** (backend only — see docs/ROADMAP.md section 2 "Current baseline" and docs/SECURITY.md section 34 for the full implementation). Delivered with a narrower rule-shape than originally sketched: `PERCENTAGE`/`FIXED`/`NONE` commission types (not "service-specific" or "tiered" as distinct strategies — a service-specific rate is expressed as a service-scoped `PERCENTAGE`/`FIXED` rule instead, via the same eight-level precedence every other scope dimension uses) and no "reversed" commission status yet, since nothing can reverse a commission until refunds themselves exist.
...
- ~~Finalized and reversed commission records with calculation snapshots.~~ Delivered as immutable `CommissionAccrual` rows, each snapshotting the exact rule terms and calculation basis used — but there is no "reversed" status yet, since reversing one presupposes a refund, which does not exist yet.
- ~~Stable receipt numbering and receipt snapshots.~~ Delivered — an atomic per-branch/year sequence (`{branchCode}-{year}-{sequence}`) and a fully immutable snapshot of business/branch/customer/line/payment details.
...
- Cash-session opening, submission, variance, approval, and rejection. **Not delivered** — reconciliation remains entirely unimplemented, deferred alongside refunds/reversals and payouts.
- ~~Owner dashboard and basic daily reports backed by verified server data.~~ Delivered as `GET .../reports/{overview,revenue,staff-performance,services,payment-methods,commissions}` — a real-time query API; no push/scheduled dashboard delivery mechanism exists (that is Phase 9's "real-time owner dashboard").

Critical tests (`apps/api/test/commission-rules.e2e-spec.ts`, `commission-accrual-posting.e2e-spec.ts`, `receipts.e2e-spec.ts`, `reports.e2e-spec.ts`):
...
- Refunds reverse affected commission correctly. **Not applicable yet** — refunds do not exist until a later phase.
- Expected cash is server-calculated. **Not applicable yet** — cash-session reconciliation does not exist yet.
...
- ~~Reports separate recorded, verified, disputed, refunded, and outstanding value.~~ Proven for recorded/verified/disputed (the overview report's `pendingPaymentClaimCount`/`disputedPaymentClaimCount`, kept strictly separate from `postedRevenue`); refunded/outstanding value reporting awaits refunds themselves.

Exit gate:

- ~~An owner can remotely reconcile a branch day and trace every displayed figure to source transactions.~~ **Partially met**: every report figure traces back to a POSTED Transaction and its snapshots, but "reconcile a branch day" in the cash-session sense (expected vs. actual cash) is not yet possible, since cash-session reconciliation itself is deferred to a later phase.
