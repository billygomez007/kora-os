...
- Android now calls the real API for one customer-facing vertical slice plus an initial business dashboard (see the Android-integration bullet above); it does not yet call the service-catalogue-management, walk-in/queue, service-session, checkout, payment, transaction, refund/reversal, cash-session, or commission-management endpoints — only discovery, availability, appointments, favorites, workspaces, and the reports overview.
...
14. Android API integration. **Started, out of sequence order, ahead of items 9–13** — see the Android-integration bullet under "Current baseline" above for exactly what is and is not connected. Remaining for this item: queue, service-session, checkout, payment, refund, cash-session, and commission operations on Android; push notifications; offline synchronization.
