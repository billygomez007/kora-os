...
- Commission accrual, service receipts, owner/manager reports, branch cash-session reconciliation, and refund/reversal corrections are all implemented (see the two "financial-integrity"/"cash controls" bullets above); payouts, commission "paid"/settlement status, payment-gateway integration, and external settlement matching do not exist yet.
...
are done — see "Current baseline" above. The next step is item 9 of the
implementation sequence: subscription billing-provider integration.
