...
- Prisma 7 foundation: `prisma.config.ts`, the full identity/tenancy/workforce/subscription/audit schema, the first migration (`foundation_identity_tenancy_subscriptions`), and an idempotent seed for permissions, system roles, entitlement definitions, and the Starter/Growth/Business/Pro/Enterprise plan records. Growth remains legacy/internal while approved public GHS prices are represented by active PlanPrice rows.
...
- No real email delivery provider is live yet. The existing `EmailOtpSender`/`SmtpEmailOtpSender` pair is prepared and tested for Resend (`smtp.resend.com`, sender `Kora OS <login@auth.koraafric.com>` on the `koraafric.com` domain — docs/ARCHITECTURE.md section 28, docs/operations/EMAIL_OTP_PRODUCTION_SETUP.md) through configuration alone, no code rewrite, but is not connected to a real account: `auth.koraafric.com` is not yet verified in Resend, no API key has been issued, and no production hosting exists to hold one. `EmailOtpSender` still fails closed in production until all of that external setup is complete; development continues to deliver through a local, credential-free Mailpit container over SMTP — see docs/SECURITY.md section 6. Phone OTP and any external identity provider (Apple, Google) remain unimplemented.
...
- Real email delivery for the OTP code. Prepared, not yet live — the existing `EmailOtpSender`/`SmtpEmailOtpSender` pair is configured and tested for Resend on `auth.koraafric.com` (docs/ARCHITECTURE.md section 28), but `EmailOtpSender` still fails closed in production until the domain is verified, a real API key is issued and stored, and production hosting exists to hold it.
