...
- The local development database used for this stage's manual emulator verification had zero seeded/published businesses, so the business-profile-through-booking-confirmation-through-appointment-detail live click-through was not driven end to end on the emulator; those flows are covered by comprehensive automated unit and Compose UI tests using fakes that mirror the real API contracts exactly, not by a live manual pass. Sign-in, session restoration, the customer navigation shell (all five tabs, including back-stack-exits-the-app), the home screen's real greeting/empty states, and search/discovery's real filters and empty states were verified live.
