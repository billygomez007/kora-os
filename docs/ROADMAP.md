...
- Production email delivery uses the provider-neutral `EmailOtpSender` boundary with Resend's HTTPS API (`EMAIL_DELIVERY_MODE=resend`, sender `Kora OS <login@koraafric.com>` on the verified `koraafric.com` domain — docs/ARCHITECTURE.md section 28, docs/operations/EMAIL_OTP_PRODUCTION_SETUP.md). Railway must hold the real Resend variables; if they are missing or incorrect, `EmailOtpSender` fails closed with a generic 503. Development continues to deliver through a local, credential-free Mailpit container over SMTP — see docs/SECURITY.md section 6. Phone OTP and any external identity provider (Apple, Google) remain unimplemented.
...
- Real email delivery for the OTP code. The code path is production-ready and uses Resend on the verified `koraafric.com` sender domain; Railway secret configuration and a live delivery test remain operational prerequisites.
